...
static void verifyInit(int line, CollisionCircle * circle, void * owner, CollisionCallback collisionCallback, Vector2x position, fixed16_16 radius, bool concave) {
	...
	TestCase_assert(circle->concave == concave, "Expected %s but got %s (line %d)", concave ? "true" : "false", circle->concave ? "true" : "false", line);
	...
	TestCase_assert(circle->lastRadius == radius, "Expected 0x%05X but got 0x%05X (line %d)", radius, circle->lastRadius, line);
}
...
static void testInit() {
	...
	CollisionCircle_init(&circle, NULL, NULL, VECTOR2x_ZERO, 0x00000, false);
	verifyInit(__LINE__, &circle, NULL, NULL, VECTOR2x_ZERO, 0x00000, false);
	...
	memset(&circle, 0x00, sizeof(circle));
	CollisionCircle_init(&circle, (void *) 0x1234, (CollisionCallback) 0x2345, VECTOR2x(0x10000, 0x20000), 0x30000, true);
	verifyInit(__LINE__, &circle, (void *) 0x1234, (CollisionCallback) 0x2345, VECTOR2x(0x10000, 0x20000), 0x30000, true);
	CollisionCircle_dispose(&circle);
	
	circlePtr = CollisionCircle_create(NULL, NULL, VECTOR2x_ZERO, 0x00000, false);
	verifyInit(__LINE__, circlePtr, NULL, NULL, VECTOR2x_ZERO, 0x00000, false);
	CollisionCircle_dispose(circlePtr);
	
	circlePtr = CollisionCircle_create((void *) 0x1234, (CollisionCallback) 0x2345, VECTOR2x(0x10000, 0x20000), 0x30000, true);
	verifyInit(__LINE__, circlePtr, (void *) 0x1234, (CollisionCallback) 0x2345, VECTOR2x(0x10000, 0x20000), 0x30000, true);
	CollisionCircle_dispose(circlePtr);
}
...
static void testUpdatePosition() {
	...
	circle = CollisionCircle_create(NULL, NULL, VECTOR2x(0x10000, 0x20000), 0x00000, false);
...
static void testUpdateRadius() {
	CollisionCircle * circle;
	
	circle = CollisionCircle_create(NULL, NULL, VECTOR2x_ZERO, 0x10000, false);
	TestCase_assert(circle->radius == 0x10000, "Expected 0x10000 but got 0x%05X", circle->radius);
	TestCase_assert(circle->lastRadius == 0x10000, "Expected 0x10000 but got 0x%05X", circle->lastRadius);
	
	CollisionCircle_updateRadius(circle, 0x00000);
	TestCase_assert(circle->radius == 0x00000, "Expected 0x00000 but got 0x%05X", circle->radius);
	TestCase_assert(circle->lastRadius == 0x10000, "Expected 0x10000 but got 0x%05X", circle->lastRadius);
	
	CollisionCircle_updateRadius(circle, 0x08000);
	TestCase_assert(circle->radius == 0x08000, "Expected 0x08000 but got 0x%05X", circle->radius);
	TestCase_assert(circle->lastRadius == 0x00000, "Expected 0x00000 but got 0x%05X", circle->lastRadius);
...
static void testInterpolate() {
	...
	circle = CollisionCircle_create(NULL, NULL, VECTOR2x(0x00000, 0x00000), 0x00000, false);
	CollisionCircle_updatePosition(circle, VECTOR2x(0x10000, 0x10000));
	CollisionCircle_updateRadius(circle, 0x10000);
	...
	TestCase_assert(circle->lastRadius == 0x08000, "Expected 0x08000 but got 0x%05X", circle->lastRadius);
	TestCase_assert(circle->radius == 0x10000, "Expected 0x10000 but got 0x%05X", circle->radius);
	...
	TestCase_assert(circle->lastRadius == 0x0C000, "Expected 0x0C000 but got 0x%05X", circle->lastRadius);
	TestCase_assert(circle->radius == 0x00000, "Expected 0x00000 but got 0x%05X", circle->radius);
